...
const filterValue = (value) => encodeURIComponent(String(value))

const publishedDateFilter = () =>
  `&or=(published_at.is.null,published_at.lte.${filterValue(new Date().toISOString())})`
...
export const supabasePublic = {
  async getPublishedBlogPosts(locale) {
    const query =
      `?select=*&locale=eq.${filterValue(locale)}&status=eq.published${publishedDateFilter()}&order=published_at.desc,created_at.desc`
    return select('blog_posts', query)
  },
  async getPublishedBlogPost(locale, slug) {
    const rows = await select(
      'blog_posts',
      `?select=*&locale=eq.${filterValue(locale)}&status=eq.published&slug=eq.${filterValue(slug)}${publishedDateFilter()}&limit=1`,
    )
    return rows?.[0] || null
  },
...
export const supabaseAdmin = {
  async listBlogPosts(locale) {
    const session = getSession()
    return request(`/rest/v1/blog_posts?select=*&locale=eq.${filterValue(locale)}&order=created_at.desc`, {
      accessToken: session?.access_token,
    })
  },
  async getBlogPost(id) {
    const session = getSession()
    const rows = await request(`/rest/v1/blog_posts?select=*&id=eq.${filterValue(id)}&limit=1`, {
      accessToken: session?.access_token,
    })
    return rows?.[0] || null
  },
  async saveBlogPost(payload) {
    ...
    if (payload.id) {
      const rows = await mutate('blog_posts', 'PATCH', body, session?.access_token, `?id=eq.${filterValue(payload.id)}`)
      return rows?.[0] || null
    }
  ...
  async deleteBlogPost(id) {
    const session = getSession()
    return remove('blog_posts', session?.access_token, `?id=eq.${filterValue(id)}`)
  },
  async listCatalogs(locale) {
    const session = getSession()
    return request(`/rest/v1/catalogs?select=*&locale=eq.${filterValue(locale)}&order=sort_order.asc,created_at.desc`, {
      accessToken: session?.access_token,
    })
  },
  async getCatalog(id) {
    const session = getSession()
    const rows = await request(`/rest/v1/catalogs?select=*&id=eq.${filterValue(id)}&limit=1`, {
      accessToken: session?.access_token,
    })
    return rows?.[0] || null
  },
  async saveCatalog(payload) {
    ...
    const body = { ...payload, updated_at: new Date().toISOString() }
    if (payload.id) {
      const rows = await mutate('catalogs', 'PATCH', body, session?.access_token, `?id=eq.${filterValue(payload.id)}`)
      return rows?.[0] || null
    }
